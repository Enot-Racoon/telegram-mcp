...
describe('MockTelegramProvider', () => {
  ...
  describe('getMessages', () => {
    it('should throw error when not authenticated', async () => {
      await expect(provider.getMessages('chat-1', 10)).rejects.toThrow('Not authenticated');
    });
  ...
  describe('sendMessage', () => {
    it('should throw error when not authenticated', async () => {
      await expect(provider.sendMessage('chat-1', 'Hello')).rejects.toThrow('Not authenticated');
    });

    it('should send a message successfully', async () => {
      await provider.login('+1234567890');
      await expect(provider.sendMessage('chat-1', 'Hello')).resolves.not.toThrow();
    });

    it('should throw error for non-existent chat', async () => {
      await provider.login('+1234567890');
      await expect(provider.sendMessage('non-existent', 'Hello')).rejects.toThrow('Chat not found');
    });
  ...
  describe('error simulation', () => {
    it('should throw simulated error when enabled', async () => {
      ...
      await expect(provider.login('+1234567890')).rejects.toThrow('Simulated Telegram API error');
    });
